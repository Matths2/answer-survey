# Accelerator Log

## Options
```json
{
  "projectName" : "friends-angular-accelerator"
}
```
## Log
```
┏ engine (Chain)
┃  Info Running Chain(GeneratorValidationTransform, UniquePath)
┃ ┏ ┏ engine.transformations[0].validated (Include)
┃ ┃ ┃  Info Will include [**]
┃ ┃ ┃ Debug .editorconfig matched [**] -> included
┃ ┃ ┃ Debug .eslintrc.json matched [**] -> included
┃ ┃ ┃ Debug .gitignore matched [**] -> included
┃ ┃ ┃ Debug .prettierignore matched [**] -> included
┃ ┃ ┃ Debug .prettierrc.json matched [**] -> included
┃ ┃ ┃ Debug .vscode/extensions.json matched [**] -> included
┃ ┃ ┃ Debug .vscode/launch.json matched [**] -> included
┃ ┃ ┃ Debug .vscode/settings.json matched [**] -> included
┃ ┃ ┃ Debug .vscode/tasks.json matched [**] -> included
┃ ┃ ┃ Debug Dockerfile matched [**] -> included
┃ ┃ ┃ Debug README.md matched [**] -> included
┃ ┃ ┃ Debug angular.json matched [**] -> included
┃ ┃ ┃ Debug catalog/catalog-info.yaml matched [**] -> included
┃ ┃ ┃ Debug config/workload.yaml matched [**] -> included
┃ ┃ ┃ Debug openapi.yaml matched [**] -> included
┃ ┃ ┃ Debug package-lock.json matched [**] -> included
┃ ┃ ┃ Debug package.json matched [**] -> included
┃ ┃ ┃ Debug src/app/app-routing.module.ts matched [**] -> included
┃ ┃ ┃ Debug src/app/app.component.css matched [**] -> included
┃ ┃ ┃ Debug src/app/app.component.html matched [**] -> included
┃ ┃ ┃ Debug src/app/app.component.spec.ts matched [**] -> included
┃ ┃ ┃ Debug src/app/app.component.ts matched [**] -> included
┃ ┃ ┃ Debug src/app/app.module.ts matched [**] -> included
┃ ┃ ┃ Debug src/assets/.gitkeep matched [**] -> included
┃ ┃ ┃ Debug src/favicon.ico matched [**] -> included
┃ ┃ ┃ Debug src/index.html matched [**] -> included
┃ ┃ ┃ Debug src/main.ts matched [**] -> included
┃ ┃ ┃ Debug src/styles.css matched [**] -> included
┃ ┃ ┃ Debug tsconfig.app.json matched [**] -> included
┃ ┃ ┃ Debug tsconfig.json matched [**] -> included
┃ ┗ ┗ Debug tsconfig.spec.json matched [**] -> included
┗ ╺ engine.transformations[1] (UniquePath)
```
